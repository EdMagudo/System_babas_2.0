...
const Search = () => {
  ...
  const handleSearch = async () => {
    try {
      ...
      const url = "https://145.223.103.28:3005/api/user/getAllNannyWith/Requirement";
      ...
      if (response.status === 200 && response.data.length === 0) {
        ...
        const nannies = response.data.map((nanny) => {
          ...
          const profilePictureUrl = fileName
            ? `https://145.223.103.28:3005/api/uploads/${fileName}`
            : "/default-profile.png";
  ...
  useEffect(() => {
    if (client.country) {
      const fetchProvinces = async () => {
        try {
          const response = await axios.get(`https://145.223.103.28:3005/api/provinces/${client.country}`);
