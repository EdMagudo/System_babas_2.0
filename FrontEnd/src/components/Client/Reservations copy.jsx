...
const Reservations = () => {
  ...
  const fetchReservations = async () => {
    ...
    if (!clientId) {
      console.error('Client ID not found in localStorage');import React, { useState, useEffect } from 'react';
      import axios from 'axios';
        ...
        const handleCancel = async (reservation_id) => {
          ...
          try {
            ...
            if (response.status === 204) {
              ...
              setMessage('Error cancelling the reservation');
            }
          } catch (error) {
        ...
        const handlePayment = async (reservation_id) => {
          try {
            const response = await axios.post(
              "http://localhost:3005/sam/pay", { reservationId: reservation_id }
            );
            const { approval_url } = response.data;
            if (approval_url) {
              window.location.href = approval_url;
            } else {
              setMessage('Error: Payment URL not found.');
            }
          } catch (error) {
        ...
        return (
          <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-lg overflow-hidden">
            <div className="p-6 border-b border-gray-100">
              <h2 className="text-2xl font-bold text-indigo-800">Your Reservations</h2>
            </div>
      
            {message && (
              <div className="p-4 my-4 text-center text-white font-semibold bg-red-500">
                {message}
              </div>
            )}
...
const handlePayment = async (reservation_id) => {
  try {
    // Send the request to the backend to create a PayPal order
    const response = await axios.post(
      "http://localhost:3005/sam/pay", { reservationId: reservation_id }
    );

    // Assuming the backend returns the PayPal approval URL
    const { approval_url } = response.data;
    ...
    if (approval_url) {
      ...
      setMessage({ text: 'Error: Payment URL not found.', type: 'error' });
    }
  } catch (error) {
    console.error('Error processing payment:', error);
