...
const ReservationList = () => {
  ...
  useEffect(() => {
    fetch("https://145.223.103.28:3005/api/reservations")
  ...
  const fetchNannyNames = async (reservations) => {
    ...
    for (const reservation of reservations) {
      ...
      if (!names[nannyId]) {
        try {
          const response = await fetch(`https://145.223.103.28:3005/api/user/${nannyId}`);
  ...
  const handlePayment = async (reservation_id) => {
    ...
    try {
      const response = await fetch(
        `https://145.223.103.28:3005/api/reservations/payClient/reservation/${reservation_id}`,
