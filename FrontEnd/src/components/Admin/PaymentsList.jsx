...
const PaymentList = () => {
  ...
  const filteredPayments = useMemo(() => {
    return payments?.filter((payment) => {
      ...
      if (start && end) {
        return paymentDate >= start && paymentDate <= new Date(end.setHours(23, 59, 59));
      } else if (start) {
    ...
    <div className="w-full bg-white rounded-2xl shadow-lg border  border-gray-100 mt-6">
      <div className="p-8">
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-2xl font-semibold text-gray-800">Lista de Pagamentos</h2>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50">
                ...
                <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Método</th>
                <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status Pagamento</th>
                <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status Reserva</th>
                <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Babá</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {filteredPayments?.map((payment) => (
                <tr key={payment.payment_id} className="hover:bg-gray-50 transition-colors duration-150">
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{payment.payment_id}</td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-3 py-1 inline-flex text-xs leading-5 font-medium rounded-full ${getStatusColor(payment.status)}`}>
                      {payment.status}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-3 py-1 inline-flex text-xs leading-5 font-medium rounded-full ${getReservationStatusColor(payment.reservation?.status)}`}>
